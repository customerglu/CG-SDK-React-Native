//
//  CustomerGluCG.swift
//  ReactNativeCustomerglu
//
//  Created by Himanshu Trehan on 26/02/25.
//
...
@objc(ReactNativeCustomerglu)
public class ReactNativeCustomerglu: RCTEventEmitter {
    @objc public static var shared: ReactNativeCustomerglu?
    ...
    @objc override public init() {
      ...
      //  CustomerGlu.getInstance.initializeSdk()
        ReactNativeCustomerglu.shared = self
...
@objc public func initCGSDK(_ flag: String) {
    ...
    @objc func catchAnalyticsNotification(notification: NSNotification) {
        if("CGBANNER_FINAL_HEIGHT" == notification.name.rawValue){
            ...
            ReactNativeCustomerglu.shared?.emitEvent(withName: notification.name.rawValue, body: height)
        }else if("CGEMBED_FINAL_HEIGHT" == notification.name.rawValue){
            if let userInfo = notification.userInfo as? [String: Any]
            {
                print(userInfo)
                let height =  CustomerGlu.embedsHeight
                ReactNativeCustomerglu.shared?.emitEvent(withName: notification.name.rawValue, body: userInfo)
            }
           
        }else{
            ReactNativeCustomerglu.shared?.emitEvent(withName: notification.name.rawValue, body: notification.userInfo)
        }
                       ...
                       if(((CGSTATE.DEEPLINK_URL == status) || (CGSTATE.SUCCESS == status)) && data != nil){
                           do{
                               ...
                               guard let dictionary = json as? [String : Any] else {
                                   ...
                                   ReactNativeCustomerglu.shared?.emitEvent(withName:"CG_UNI_DEEPLINK_EVENT", body: jsonString1)
                                   return
                               }
                               let jsonObject = [
